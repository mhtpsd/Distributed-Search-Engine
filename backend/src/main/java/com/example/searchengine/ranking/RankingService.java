package com.example.searchengine.ranking;
...
public class RankingService {
    ...
    private double computeTfIdfForPage(Page page, Set<String> queryTokens) {
        double score = 0.0;
        int totalDocs = indexerService.invertedIndex.getTotalDocuments();
        for (String token : queryTokens) {
            int tf = indexerService.invertedIndex.getTermFrequencies(token).getOrDefault(page.getId(), 0);
            int df = indexerService.invertedIndex.getDocumentFrequency(token);
    ...
    private double computeTitleBoost(Page page, Set<String> queryTokens) {
        Set<String> titleTokens = com.example.searchengine.util.TokenizerUtil.tokenize(page.getTitle());
